...
TEST(ParserSyntaxBaselineTest, RejectsQuotedEmptyPercentProgramNameMetadata) {
  const auto result = gcode::parse("%\"\"\nG1 X1\n");
  EXPECT_FALSE(result.program.program_name.has_value());
  ASSERT_FALSE(result.diagnostics.empty());
  EXPECT_EQ(result.diagnostics[0].location.line, 1);
...
TEST(ParserSyntaxBaselineTest, RejectsSymbolOnlyPercentProgramNameMetadata) {
  const auto result = gcode::parse("%/\nG1 X1\n");
  EXPECT_FALSE(result.program.program_name.has_value());
  ASSERT_FALSE(result.diagnostics.empty());
  EXPECT_EQ(result.diagnostics[0].location.line, 1);
...
TEST(ParserSyntaxBaselineTest, RejectsColonPrefixedPercentProgramNameMetadata) {
...
TEST(ParserSyntaxBaselineTest, RejectsSlashPrefixedPercentProgramNameMetadata) {
  const auto result = gcode::parse("%/MPF1000\nG1 X1\n");
  EXPECT_FALSE(result.program.program_name.has_value());
  ASSERT_FALSE(result.diagnostics.empty());
  EXPECT_EQ(result.diagnostics[0].location.line, 1);
...
TEST(ParserSyntaxBaselineTest, RejectsDotPrefixedPercentProgramNameMetadata) {
  const auto result = gcode::parse("%..MPF1000\nG1 X1\n");
  EXPECT_FALSE(result.program.program_name.has_value());
  ASSERT_FALSE(result.diagnostics.empty());
  EXPECT_EQ(result.diagnostics[0].location.line, 1);
...
TEST(ParserSyntaxBaselineTest, RejectsRepeatedDotPercentProgramNameMetadata) {
  const auto result = gcode::parse("%A..B\nG1 X1\n");
  EXPECT_FALSE(result.program.program_name.has_value());
  ASSERT_FALSE(result.diagnostics.empty());
  EXPECT_EQ(result.diagnostics[0].location.line, 1);
...
TEST(ParserSyntaxBaselineTest,
     RejectsRepeatedHyphenPercentProgramNameMetadata) {
  const auto result = gcode::parse("%A--B\nG1 X1\n");
  EXPECT_FALSE(result.program.program_name.has_value());
  ASSERT_FALSE(result.diagnostics.empty());
  EXPECT_EQ(result.diagnostics[0].location.line, 1);
...
TEST(ParserSyntaxBaselineTest,
     RejectsRepeatedUnderscorePercentProgramNameMetadata) {
  const auto result = gcode::parse("%A__B\nG1 X1\n");
  EXPECT_FALSE(result.program.program_name.has_value());
  ASSERT_FALSE(result.diagnostics.empty());
  EXPECT_EQ(result.diagnostics[0].location.line, 1);
  EXPECT_EQ(result.diagnostics[0].location.column, 1);
...
TEST(ParserSyntaxBaselineTest,
     RejectsMixedAdjacentPunctuationPercentProgramNameMetadata) {
  const auto result = gcode::parse("%A.-B\nG1 X1\n");
  EXPECT_FALSE(result.program.program_name.has_value());
  ASSERT_FALSE(result.diagnostics.empty());
  EXPECT_EQ(result.diagnostics[0].location.line, 1);
...
TEST(ParserSyntaxBaselineTest, TrimsLeadingWhitespaceFromPercentProgramName) {
  const auto result = gcode::parse("%   MPF1000\nG1 X1\n");
  ASSERT_TRUE(result.diagnostics.empty());
  ASSERT_TRUE(result.program.program_name.has_value());
  EXPECT_EQ(result.program.program_name->raw_text, "%   MPF1000");
...
TEST(ParserSyntaxBaselineTest, TrimsLeadingTabFromPercentProgramName) {
  const auto result = gcode::parse("%\tMPF1000\nG1 X1\n");
  ASSERT_TRUE(result.diagnostics.empty());
  ASSERT_TRUE(result.program.program_name.has_value());
  EXPECT_EQ(result.program.program_name->raw_text, "%\tMPF1000");
