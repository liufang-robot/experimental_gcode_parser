...
- Updated the contract runner so every read attempt emits a
  `system_variable_read` event, including fresh retry events after
  `resume_blocked`.
- Updated the execution-contract docs/spec to describe outcome-aware read
  traces before tester-owned reviewed fixtures are added.
...
Known limitations:
- This slice changes the fixture schema and runner behavior only; persistent
  reviewed pending/error read fixtures remain tester-owned follow-up work.
- `runtime.system_variables` remains ready-only shorthand for fixed-value
  cases.
- Typed variable values and runtime-backed writes remain deferred.
...
- `./build/execution_contract_runner_tests --gtest_filter='ExecutionContractRunnerTest.RuntimeReadScriptCanBlockAndResumeWithPerAttemptTraceEvents:ExecutionContractRunnerTest.RuntimeReadScriptCanFaultWithPerAttemptErrorTraceEvent'`
...
- Updated the reviewed core execution-contract fixtures that already depend on
  ...
  `system_variable_read` events ahead of the existing motion/branch outcome.
- Kept the enforced core dataset unchanged; this tester pass only refreshed the
  stale reviewed traces for existing cases rather than promoting a new
  repeated-read core fixture.
...
- Fixtures:
  ...
  - `testdata/execution_contract/core/if_system_variable_false_branch.events.yaml`

Known limitations:
- Persistent reviewed repeated-read cases are still optional tester follow-up;
  this slice keeps them out of the enforced core dataset.
- Runtime-backed writes and indexed selector forms remain deferred.
...
- Promoted a persistent reviewed `linear_move_cancelled` execution-contract
  ...
  `linear_move` -> `blocked` -> `cancelled` path.
- Extended the core contract runner test list and review docs so cancelled is
  enforced alongside the existing blocked and resume async cases.

SPEC sections / tests:
- SPEC: Section 6.2 execution contract fixture baseline
- Tests: `test/execution_contract_runner_tests.cpp`
- Fixtures/docs:
  - `testdata/execution_contract/core/linear_move_cancelled.ngc`
  - `testdata/execution_contract/core/linear_move_cancelled.events.yaml`
  - `testdata/execution_contract/README.md`
  - `docs/src/execution_contract_review.md`
...
Known limitations:
- The review subsite is generated from the currently supported core execution
